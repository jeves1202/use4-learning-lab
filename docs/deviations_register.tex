...
documents (\texttt{estu\_audit.tex}, \texttt{daily\_panel\_audit.tex}, and the
twelve \texttt{<factor>\_factor\_audit.tex} files). Those audits answer
...
\texttt{beta} $\to$ \texttt{nlb};\quad \texttt{size} $\to$ \texttt{nls}.

%──────────────────────────────────
\subsection{The reference run}

The numbers in this register come from the lab's reference run of 2026-06-11
(Sharadar data through 2026-06-10): all fourteen builds executed sequentially in
dependency order, every validation green. Your rebuild will differ in row counts
and tail statistics with data vintage; the structural signatures (stability
coefficients, monotone quintiles, standardisation identities) should not.

%──────────────────────────────────────────────────────────────────────────────
\section{Headline Fidelity Gaps}

Four deviations are material enough to frame the whole register. All four are
...
\begin{description}[style=nextline]
  ...
  \item[Solution] Ken French daily RF; benchmark $=$ ESTU cap-weighted excess
    ...
    accrues), built once in \texttt{daily\_panel\_build.ipynb}.
...
\begin{description}[style=nextline]
  ...
  \item[Solution] \texttt{daily\_panel\_build.ipynb} builds the panel once
...
C1 & Productionising & Move shared loaders/validation helpers into a real package; update your build tooling \\
\bottomrule
\end{longtable}
...
\texttt{daily\_panel\_audit.tex}, and \texttt{<factor>\_factor\_audit.tex} in each
factor directory --- all regenerated from the same 2026-06-11 run audited here.
Construction rationale at spec level: \texttt{<factor>\_spec.ipynb}
(NOT-IN-PDF master lists), \texttt{estu/estu\_spec.ipynb},
\texttt{daily/daily\_panel\_spec.ipynb}.
