...
\begin{enumerate}[leftmargin=1.6em,nosep]
  \item \textbf{EWMA volatility and correlation.} Exponentially weighted moving averages let recent observations matter more. Volatilities and correlations are estimated \emph{separately}---a shorter half-life ($84$ days) for the volatilities and a longer one ($252$ days) for the correlation matrix, then recombined into the covariance---because volatility moves faster than co-movement.
  \item \textbf{Newey--West autocorrelation correction} (lag $5$). Monthly factor returns carry serial correlation; ignoring it understates longer-horizon variance. The Newey--West adjustment folds in the lagged autocovariances.
...
\section{What the model gets right, and an honest caveat}\label{sec:caveat}

\paragraph{Learning objectives.}
\begin{itemize}[leftmargin=1.4em,nosep]
  \item Recite the right-to-left compositional read of the pipeline.
  \item State the master identity in one sentence.
  \item List the model's principal honest limitations.
\end{itemize}

\begin{intuition}
Read the line right to left. The portfolio risk $\sigma_p$ is a quadratic form in the four built objects. $\Fmat$ and $\Del$ are forecasts of two covariances---the factor returns' and the specific returns'---both produced by the cross-sectional regression. That regression needs the exposure matrix $\Xmat$ and a clean return panel, defined over the estimation universe. And the universe and the panel need point-in-time data. Pull any link and the chain breaks; assemble them in order and one risk number falls out the end.
\end{intuition}
...
A tiny sketch of the final composition, mirroring the build with a dense $\Fmat$. No proprietary constants appear. All group contributions are in volatility units and sum to \texttt{sigma\_p}.
...
References: Menchero, Orr \& Wang (2011), \textit{The Barra US Equity Model (USE4): Methodology Notes} and \textit{Empirical Notes}; Menchero, Wang \& Orr (2011), \textit{Eigen-Adjusted Covariance Matrices}. Empirical figures are from a personal first-principles build (factor-return panel through 2026-04-30; risk-decomposition snapshot 2026-03-31).
\end{center}
